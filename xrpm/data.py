...
class Output(BaseModel):
    ...
    def _parse_outputs(text: str) -> Iterable["Output"]:
        ...
        for match in matches:
            ...
            yield Output(name=name, connected=connected,
                         display=Display(edid=edid))

    @staticmethod
    def load_outputs() -> list["Output"]:
        p = run(["xrandr", "--properties"], stdout=PIPE, text=True, check=True)
        return list(Output._parse_outputs(str(p.stdout)))
...
class Profile(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        if self.match_outputs:
            displays = [
                f"{output}: {display}" for output, display in displays
            ]
        else:
        ...
        try:
            with open(path, "r") as file:
                ...
                if not isinstance(data, dict) \
                        or not all(map(lambda k: isinstance(k, str),
                                       data.keys())):
    ...
    def save(profiles: Profiles, path: str) -> None:
        with open(path, "w") as file:
            dump({ name: profile.model_dump()
                   for name, profile in profiles.items() }, file, indent=4)
